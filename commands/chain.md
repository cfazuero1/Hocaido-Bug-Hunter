...
## Caido Integration for Chain Testing

Use caido-client.ts to test B candidates while preserving session auth:

```bash
CAIDO="npx tsx ~/.claude/skills/caido-mode/caido-client.ts"

# Find related endpoints from proxy history (avoids manual auth header hunting)
$CAIDO search 'req.host.cont:"target.com" AND req.path.cont:"/api/"' --limit 50

# Test IDOR B candidate: change the victim ID, keep attacker's auth
$CAIDO edit <request-id-A> --path /api/user/VICTIM_ID
$CAIDO edit <request-id-A> --replace "attacker_id:::victim_id"

# Test privilege escalation B candidate
$CAIDO edit <request-id-A> --method POST --body '{"role":"admin"}'

# For OAuth chain: find redirect_uri parameter in traffic
$CAIDO search 'req.query.cont:"redirect_uri" OR req.path.regex:"/(oauth|callback)/"' --limit 10

# For XSS→ATO: check if admin views the feature
$CAIDO search 'req.path.cont:"/admin"' --limit 20

# Name replay sessions for each chain hop
$CAIDO create-session <request-id-A>
$CAIDO rename-session <session-id> "chain-A-idor"
$CAIDO create-session <request-id-B>
$CAIDO rename-session <session-id-2> "chain-B-privesc"

# Log confirmed chain as a Caido finding
$CAIDO create-finding <request-id-B> \
  --title "Chain: [A-class] → [B-class] = [impact]" \
  --description "A: [endpoint-A]. B: [endpoint-B]. Combined impact: [impact]."
```
