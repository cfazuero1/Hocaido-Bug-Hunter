...
## Caido PoC Generation

If Caido is running, generate the PoC curl command and log the finding automatically:

```bash
CAIDO="npx tsx ~/.claude/skills/caido-mode/caido-client.ts"

# Pull exact raw request for Steps to Reproduce section
$CAIDO get <request-id>
$CAIDO get-response <request-id> --compact

# Generate copy-paste curl command for the PoC
$CAIDO export-curl <request-id>

# Log to Caido Findings tab with dedup key (prevents duplicates on re-runs)
$CAIDO create-finding <request-id> \
  --title "[BugClass] in [endpoint] allows attacker to [impact]" \
  --description "[impact and CVSS]" \
  --reporter "$(whoami)" \
  --dedupe-key "[target]-[bugclass]-[endpoint]"
```
