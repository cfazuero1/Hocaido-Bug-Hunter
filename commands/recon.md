...
### Step 0: Check Caido Proxy History (skip if Caido not running)

Before running passive tools, check if you already have traffic for this target:
...
# Pull all hosts already seen for this target
$CAIDO search "req.host.cont:\"$TARGET\"" --limit 100

# Flag interesting responses already captured: 500s, redirects, large JSON
$CAIDO search "req.host.cont:\"$TARGET\" AND (resp.code.gte:500 OR resp.len.gt:50000)" --limit 20

# Find API endpoints already visited
$CAIDO search "req.host.cont:\"$TARGET\" AND req.path.cont:\"/api/\"" --limit 50

# Save recon filter preset for fast reuse during hunting
$CAIDO create-filter "$TARGET-api" --query "req.host.cont:\"$TARGET\" AND req.path.cont:\"/api/\""
```

Hosts found in proxy history but missing from subdomain enum = potential staging or internal assets.
