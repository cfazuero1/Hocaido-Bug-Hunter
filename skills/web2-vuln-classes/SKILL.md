...
### Testing
```bash
# Caido Automate — last-byte sync (use Batch Send with synchronized gate)
# Python parallel
...
### Detection
```
1. Caido: HTTP Request Smuggler workflow
2. Send to Automate → HTTP Request Smuggler → Smuggle probe
...
# Param Miner (standalone) — finds unkeyed headers automatically
Caido Automate: fuzz headers with param-miner wordlist
```
...
```
1. Enter wrong OTP → intercept response in Caido
...
### Tools
```bash
# SAML Raider (standalone) — automated XSW testing
# Intercept SAMLResponse in Caido → decode and modify manually
