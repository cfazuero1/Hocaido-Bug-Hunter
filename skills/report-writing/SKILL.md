...
## CAIDO — GENERATE PoC FROM HTTP HISTORY

Always pull the confirmed request from Caido rather than reconstructing it manually.

```bash
# Get the exact raw request for Steps to Reproduce
npx tsx ~/.claude/skills/caido-mode/caido-client.ts get <request-id>

# Get the response (shows actual impact)
npx tsx ~/.claude/skills/caido-mode/caido-client.ts get-response <request-id> --compact

# Export as curl command (paste directly into report)
npx tsx ~/.claude/skills/caido-mode/caido-client.ts export-curl <request-id>

# Export as Python Requests (use convert workflow p:26 in Replay tab)
# Use Copy As Python Requests workflow for Python PoC

# Pull all Caido findings for this hunt (cross-check before reporting)
npx tsx ~/.claude/skills/caido-mode/caido-client.ts findings --limit 50

# Create replay session for triager replication
npx tsx ~/.claude/skills/caido-mode/caido-client.ts create-session <request-id>
npx tsx ~/.claude/skills/caido-mode/caido-client.ts rename-session <session-id> "vuln-poc-[bug-class]"
```

**In the report's Steps to Reproduce, use the curl output directly** — it includes all headers, cookies, and body exactly as captured. This proves the bug is real and makes it trivial for triagers to replicate.
...
## HACKERONE SUBMISSION — HUMAN APPROVAL REQUIRED

**NEVER submit without explicit human confirmation. Always show the full draft first.**

### Submission Flow

```
1. get_program_scope      → find correct scope asset ID for the vulnerable endpoint
2. get_program_weaknesses → find correct CWE/weakness ID for the bug class
3. SHOW FULL DRAFT        → display everything to human for review
4. WAIT FOR APPROVAL      → do not proceed until human says "submit" or "go"
5. submit_report          → only after explicit approval
```

### Pre-Submission Checklist (verify before showing draft)
```
[ ] Title follows formula: [Class] in [endpoint] allows [actor] to [impact]
[ ] vulnerability_information has Steps to Reproduce with copy-paste HTTP request
[ ] impact field is specific — no "could potentially" language
[ ] severity_rating matches CVSS score calculated
[ ] structured_scope_id confirmed against get_program_scope output
[ ] weakness_id confirmed against get_program_weaknesses output
[ ] Caido export-curl output included in Steps to Reproduce
[ ] Two test accounts documented (attacker + victim)
[ ] Expected vs Actual behavior stated
```

### Draft Display Format (show to human before submitting)
...
Weakness:   [weakness name] (ID: [weakness_id])
Scope:      [asset] (ID: [structured_scope_id])
...
══════════════════════════════════════════════
Submit this report? Type "submit" to confirm or "edit" to revise.
══════════════════════════════════════════════
```

### After Submission
```bash
# Save report ID and URL returned by submit_report
# Example response: { "id": "12345", "url": "https://hackerone.com/reports/12345" }

# Log the submission to hunt memory
/remember — include report URL, program, bug class, endpoint, severity
```
