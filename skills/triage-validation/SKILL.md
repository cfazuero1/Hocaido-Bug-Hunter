...
**Caido Evidence Pull (run before Gate 0):**
```bash
# Pull the exact request/response for the finding
npx tsx ~/.claude/skills/caido-mode/caido-client.ts get <request-id>
npx tsx ~/.claude/skills/caido-mode/caido-client.ts get-response <request-id>

# Export curl command for PoC (paste into report)
npx tsx ~/.claude/skills/caido-mode/caido-client.ts export-curl <request-id>

# Check if passive workflows flagged the same endpoint (corroborating signal)
npx tsx ~/.claude/skills/caido-mode/caido-client.ts findings --limit 20

# Check SecretSniffer/Leakz passive hits on this host
npx tsx ~/.claude/skills/caido-mode/caido-client.ts search 'source:"workflow" AND req.host.eq:"target.com"' --limit 20

# Create a named finding in Caido to track it
npx tsx ~/.claude/skills/caido-mode/caido-client.ts create-finding <request-id> \
