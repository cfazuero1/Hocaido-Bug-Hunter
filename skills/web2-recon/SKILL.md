...
### Minutes 25-30: Manual Exploration + Caido Plugin Activation

Open Caido. Before browsing, confirm these passive workflows are enabled (they auto-run on all traffic):
- `p:4` SecretSniffer — API keys/tokens in responses
- `p:12` Leakz — data leaks
- `p:6` Redirect To Parameter Value — open redirect detection
- `p:8` OWASP Top 25 Vulnerable Parameters — flags known vuln params
- `p:21` GAP "Sus" Params — suspicious parameter names
- `p:22`/`p:23` FindSSO / EvalSSO — SSO flow detection
- `p:1` Url In Parameter — SSRF/open redirect surface

Browse the app with proxy on:
...
After browsing, check passive workflow findings:
```bash
# Check what passive workflows flagged during manual browsing
npx tsx ~/.claude/skills/caido-mode/caido-client.ts search 'source:"workflow"' --limit 50

# Check SecretSniffer/Leakz hits specifically
npx tsx ~/.claude/skills/caido-mode/caido-client.ts findings --limit 20
```

Enable active plugins for discovered surface:
- **JS Analyzer + jxscout** — on every JS-heavy endpoint (Next.js, React SPAs)
- **ParamFinder** — run on API endpoints to discover hidden parameters
- **GraphQL Analyzer** — if GraphQL detected, use `p:15` Introspection Query convert workflow
- **Scanner** — passive vuln detection across all traffic
- **RetireJS Scanner** — flags outdated JS libraries with known CVEs
- **CSP Auditor** — checks Content-Security-Policy headers on every response
...
### Caido Post-Recon Triage

After manual browsing, run these to extract high-signal items from collected traffic:

```bash
# API endpoints discovered
npx tsx ~/.claude/skills/caido-mode/caido-client.ts search 'req.path.cont:"/api/"' --limit 100

# ID parameters in URLs (IDOR surface)
npx tsx ~/.claude/skills/caido-mode/caido-client.ts search 'req.path.regex:"/[0-9]+"' --limit 50

# Auth endpoints
npx tsx ~/.claude/skills/caido-mode/caido-client.ts search 'req.path.regex:"/(login|auth|oauth|token|sso|saml|callback)"' --limit 30

# GraphQL
npx tsx ~/.claude/skills/caido-mode/caido-client.ts search 'req.path.cont:"graphql"' --limit 20

# Error responses (misconfigs, stack traces)
npx tsx ~/.claude/skills/caido-mode/caido-client.ts search 'resp.code.gte:500' --limit 20

# Large responses (potential data exposure)
npx tsx ~/.claude/skills/caido-mode/caido-client.ts search 'resp.len.gt:50000' --limit 20

# Save useful filters for the hunt
npx tsx ~/.claude/skills/caido-mode/caido-client.ts create-filter "API Endpoints" --query 'req.path.cont:"/api/"'
npx tsx ~/.claude/skills/caido-mode/caido-client.ts create-filter "ID Parameters" --query 'req.path.regex:"/[0-9]+"'
npx tsx ~/.claude/skills/caido-mode/caido-client.ts create-filter "Auth Flows" --query 'req.path.regex:"/(login|auth|oauth|token|callback)"'
npx tsx ~/.claude/skills/caido-mode/caido-client.ts create-filter "Errors" --query 'resp.code.gte:500'
```

Run CORS Checker on all API origins found:
```bash
# Find an authenticated API request, then run CORS Checker (active workflow p:25)
npx tsx ~/.claude/skills/caido-mode/caido-client.ts search 'req.path.cont:"/api/" AND resp.code.eq:200' --limit 5
# Use the request ID with edit to test CORS: add Origin: https://evil.com header
