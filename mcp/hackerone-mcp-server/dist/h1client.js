...
async function h1Fetch(path, params, options) {
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
        ...
        try {
            ...
            lastErr = err;
            if (err.message?.includes("HackerOne API error"))
                throw err;
        }
    }
    throw lastErr ?? new Error("h1Fetch failed after retries");
}
async function h1Post(path, body, contentType = "application/json") {
...
async function h1FetchAllPages(path, extraParams, maxPages = 20) {
    const all = [];
...
export async function searchReports(opts = {}) {
    const needsFilter = !!(opts.program ||
        opts.severity ||
        opts.state ||
        opts.query);
    ...
    if (needsFilter) {
        ...
        if (hasServerFilters) {
            ...
            for (let page = 1; page <= 20; page++) {
                ...
                if (data.data.length < 100)
                    break;
                if (!opts.query && allReports.length >= requestedSize)
                    break;
            }
        }
        else {
            ...
            if (probeRes.data?.length === 100) {
                ...
                for (let page = lo; page >= 1; page--) {
                    ...
                    if (!data.data || data.data.length === 0)
                        continue;
                    allReports.push(...data.data);
                    const tempFiltered = allReports.filter((r) => {
                        const q = opts.query.toLowerCase();
                        const title = r.attributes.title?.toLowerCase() ?? "";
                        const vuln = r.attributes.vulnerability_information?.toLowerCase() ?? "";
                        const weakness = r.relationships?.weakness?.data?.attributes?.name?.toLowerCase() ??
                            "";
                        return (title.includes(q) || vuln.includes(q) || weakness.includes(q));
                    });
                    if (tempFiltered.length >= requestedSize)
                        break;
                }
            }
            else {
    ...
    let reports = allReports.map((r) => mapReportSummary(r));
...
export async function getReport(reportId) {
    ...
    return {
        ...
        program: r.relationships?.program?.data?.attributes?.handle ?? null,
        structured_scope: r.relationships?.structured_scope?.data?.attributes?.asset_identifier ??
            null,
        structured_scope_type: r.relationships?.structured_scope?.data?.attributes?.asset_type ?? null,
...
export async function getReportActivities(reportId, _pageSize = 50) {
    const data = await h1Fetch(`/hackers/reports/${reportId}`);
    ...
    return activities.map((a) => ({
        ...
        actor: a.relationships?.actor?.data?.attributes?.username ??
...
// ── Get program details ───────────────────────────────────────────
export async function getProgramDetails(handle) {
    ...
    return {
        ...
        average_time_to_first_program_response: attrs.average_time_to_first_program_response,
...
export async function getProgramScope(handle, pageSize = 100) {
    const allData = await h1FetchAllPages(`/hackers/programs/${handle}/structured_scopes`);
    const scopes = allData.map((s) => ({
        id: s.id,
...
export async function getProgramWeaknesses(handle, pageSize = 100) {
    const allData = await h1FetchAllPages(`/hackers/programs/${handle}/weaknesses`);
    const weaknesses = allData.map((w) => ({
        id: w.id,
...
export async function getReportSummary(reportId) {
    ...
    const comments = activities.filter((a) => a.message &&
...
export async function addComment(reportId, message, internal = false) {
    const body = {
        data: {
    ...
    const result = await h1Post(`/hackers/reports/${reportId}/activities`, body);
    return {
        id: result.data?.id,
...
export async function closeReport(reportId, message) {
    const body = {
        data: {
...
export async function searchDisclosedReports(opts) {
    ...
    let reports = (data.data ?? []).map((r) => ({
        ...
        program: r.relationships?.team?.data?.attributes?.handle ??
