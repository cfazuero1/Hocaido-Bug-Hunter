...
Use caido-client.ts to auto-populate the report with real request/response data:

```bash
CAIDO="npx tsx ~/.claude/skills/caido-mode/caido-client.ts"

# Find the exact request in proxy history
$CAIDO search 'req.path.cont:"/the/endpoint"' --limit 10

# Pull full raw request for Steps to Reproduce
$CAIDO get <request-id>

# Pull response for impact evidence
$CAIDO get-response <request-id> --compact

# For multi-step chains, pull each hop's request in sequence
$CAIDO get <request-id-step1>
$CAIDO get <request-id-step2>

# Generate ready-to-paste curl command for the PoC section
$CAIDO export-curl <request-id>

# Log the finding to Caido's Findings tab with dedup key
$CAIDO create-finding <request-id> \
  --title "[BugClass] in [endpoint] allows attacker to [impact]" \
  --description "[full impact description]" \
  --dedupe-key "[target]-[bug-class]-[endpoint-hash]"
```
