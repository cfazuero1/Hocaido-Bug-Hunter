...
At Gate 0, use caido-client.ts to pull and re-verify the finding without asking the researcher to paste requests manually:
...
# Pull full request + response for the finding
$CAIDO get <request-id>
$CAIDO get-response <request-id> --compact

# Replay to confirm still reproducible RIGHT NOW
$CAIDO replay <request-id>

# For IDOR — change the ID parameter to victim's and replay
$CAIDO edit <request-id> --replace "attacker_id:::victim_id"

# Log confirmed finding to Caido's Findings tab
$CAIDO create-finding <request-id> --title "[BugClass] on [endpoint]" --description "[impact]"
```

For OOB findings (SSRF, blind injection): use Interactsh — Caido has no built-in OAST.

If Caido is NOT running:
- Ask the researcher to paste the HTTP request/response manually
- Use Interactsh or webhook.site for OOB confirmation
