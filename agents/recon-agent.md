...
# Pull proxy history already collected for this target
$CAIDO search "req.host.cont:\"$TARGET\"" --limit 100

# Flag interesting responses: 500s, redirects, large JSON payloads
$CAIDO search "req.host.cont:\"$TARGET\" AND (resp.code.gte:500 OR resp.len.gt:50000)" --limit 20

# Uncover endpoints not found by passive tools
$CAIDO search "req.host.cont:\"$TARGET\" AND req.path.cont:\"/api/\"" --limit 50
```

Cross-reference results: hosts found in proxy history that aren't in subdomain enum output may reveal internal or staging assets. Prioritize unvisited subdomains in the attack surface ranking.

If Caido is NOT running, skip this section entirely — all recon works without it.
