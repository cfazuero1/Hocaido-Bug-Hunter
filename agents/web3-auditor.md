...
## Caido Integration (optional — only if protocol has a web frontend)

```bash
CAIDO="npx tsx ~/.claude/skills/caido-mode/caido-client.ts"

# Check proxy history for API calls to the protocol's backend/indexer
$CAIDO search "req.host.cont:\"protocol-name\"" --limit 50

# Look for GraphQL endpoints or admin panels in traffic
$CAIDO search 'req.path.cont:"/graphql" OR req.path.cont:"/admin"' --limit 20

# Hunt off-chain API calls that match on-chain function names
$CAIDO search '"withdraw" OR "deposit" OR "claim" OR "harvest"' --limit 30

# Check for auth bypass on off-chain endpoints
$CAIDO edit <admin-request-id> --remove-header "Authorization"
```

If Caido is NOT running, skip this section — web3 auditing is primarily on-chain analysis.
